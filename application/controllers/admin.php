...
class admin extends Home
{
    ...
    */
    public function config_member()
    {
        ...
        $crud->columns('name', 'member_idd', 'user_type', 'type_id', 'email', 'mobile', 'address', 'status');

        $crud->required_fields('name', 'member_idd','user_type','email', 'mobile', 'password', 'address', 'status');

        $crud->add_fields('name', 'user_type', 'type_id', 'email', 'member_idd', 'mobile', 'password', 'address', 'status');

        $crud->edit_fields('name', 'member_idd', 'user_type', 'type_id',  'email', 'mobile', 'address', 'status');
        ...
        $crud->set_rules('member_idd',$this->lang->line("member id"), 'required|is_unique[member.member_idd]');
        $crud->set_rules('mobile',$this->lang->line("mobile"), 'required|is_unique[member.mobile]');
        ...
        $crud->display_as('member_idd', $this->lang->line("member id"));
    ...
    */
    ...
    public function circulation_data()
    {
        ...
        $member_id = trim($this->input->post('member_id', true));
        ...
        if ($is_searched) {
            ...
            $this->session->set_userdata('circulation_data_member_id', $member_id);
        ...
        $search_member_id = $this->session->userdata('circulation_data_member_id');
        ...
        if ($search_member_id) {
            $where_simple['member.member_idd like'] = "%".$search_member_id."%";
        }
    ...
    */
    public function requested_books_data()
    {
        ...
        $order_by_str=$sort." ".$order;
            ...
            // setting properties for search
        $member_name = trim($this->input->post('name', true));
        $member_idd = trim($this->input->post('member_id', true));
        $book_title  = trim($this->input->post('book_title', true));
        $author      = trim($this->input->post("author", true));
        $from_date = $this->input->post('from_date', true);
        if($from_date != '')
        ...
        if ($is_searched) {
            ...
            $this->session->set_userdata('requested_books_member_idd',    $member_idd);
        ...
        $search_member_id = $this->session->userdata('requested_books_member_idd');
        ...
        $offset = ($page-1)*$rows;
        $result = array();
        $select = array('request_book.id','member.name','member.member_idd','request_book.book_title','request_book.author','request_book.edition','request_book.note','request_book.request_date','request_book.edition','request_book.request_status');
...
//End of the section of delete first row. *************************
   ...
   public function generate_id_data()
   {
        ...
        $member_idd  = trim($this->input->post("member_idd", true));
        ...
        if($to_date != '')
	 	...
	 	if ($is_searched) {
            ...
            $this->session->set_userdata('member_search_member_idd',         $member_idd);
        ...
        $search_member_idd       = $this->session->userdata('member_search_member_idd');
      ...
      // trimming data
        ...
        if ($search_member_idd) {
            $where_simple['member_idd']    = "%".$member_idd."%";
        }
	 	...
	 	$select = array('member.id as idd','member.id as id','name','member.member_idd as member_idd','email','user_type','mobile','address','add_date','member_type.member_type_name as type_id','status');
    	$join =  $join = array("member_type" => "member.type_id = member_type.id,left");        
   ...
   public function barcode_generate_id(){
   	if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        {           
            ...
            $src=base_url()."barcode.php?code=".$barcode['member_idd'];
            $class="pull-left";
            $style="";
            ...
            $str .= "<div class='row clearfix' style='width:100%;min-height:2.3in !important;margin-left:.15in;'>";
                $str .= "<div class='clearfix {$class}' style='padding:10px;width:3.36in !important;height:2.125in !important;border:1px solid gray; {$style}'>
                        ...
                        <p><b>".$this->lang->line("ID")." : </b>".$barcode['member_idd']."</p>
                        <p><b>".$this->lang->line("Designation")." : </b>".$barcode['member_type']."</p>
